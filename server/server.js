...
server.get('/api/v1/users/:userId', async (req, res) => {
  ...
  const viewUser = users.filter((el) => el.id === +userId)
  res.json(viewUser)
}) // read

server.post('/api/v1/users', async (req, res) => {
  const newUser = req.body // { "name": "ivan" }
  const users = await rFile() // all obj
  const newId = users[users.length - 1].id + 1 // lastIndex
...
server.patch('/api/v1/users/:userId', async (req, res) => {
  ...
  const users = await rFile()
  const filteredUser = users.reduce((acc, rec) => {
    if (rec.id === +userId) {
      return [...acc, { ...rec, ...updUser }]
    }
    return [...acc, rec]
  }, [])
  await wrFile(filteredUser)
